...
Ensure the application includes the following features:
...
4. Native Streamlit data visualizations (such as bar charts or line charts) generated by parsing or leveraging the semantic data returned by the agent to visually analyze the query results.
5. A sidebar with these 5 suggested starter questions:
...
6. A loading spinner while the agent is thinking
7. The app must use get_active_session() to connect to Snowflake

Make sure the app works flawlessly in Streamlit in Snowflake (SiS) 
...
Deploy it to Snowflake in the RETAIL_MAX database.
```
